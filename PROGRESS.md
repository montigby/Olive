...
**2026-08-20: orphaned test-family cleanup confirmed done.** Ran a fresh diagnostic query (test/audit/demo name match, `+alias` admin emails, member count ≤1) — found 4 candidate family units, not the 2 originally suspected. Two were genuinely orphaned and deleted: "Email Delivery Test Family" (`smithjac007+oliveadmintest@gmail.com`, 1 member, never logged in — the exact orphan flagged since 2026-07-25) and "The Test Family" (`test@olive.com`, obviously a placeholder, 1 member, never logged in). "Smith Family" (9 members, real activity through 2026-08-14) is the standing real test/dev unit — untouched. **"The Ahrendsen Family" (`ashlynahr@gmail.com`, created 2026-08-04, 1 member, never logged in) does not match any known test pattern and the user is "almost positive it's real" — left alone, deliberately not deleted.** This is plausibly the product's first real, unprompted signup from the live public landing page (self-serve "Create Directory" flow) — worth knowing about, not a bug or debris. `family_units.id` cascades to `persons`/`accounts` on delete, so a plain `DELETE FROM family_units WHERE id IN (...)` was sufficient for the two real orphans.

A broader, deferred sweep of *all* test accounts/families (not just these) is still explicitly wanted by the user "sometime later" — not urgent, no timeline set.
...
**2026-08-20: no-login memory-prompt unsubscribe shipped.** Closes the second of the two gaps `legacy_memories_feature.md` flagged 2026-08-14 against the original spec (reply-to-email remains the one open gap — see Backlog below). Every memory-prompt email now carries a one-click unsubscribe link built from a new, narrow-purpose JWT (`signMemoryPromptUnsubscribeToken`/`verifyMemoryPromptUnsubscribeToken` in `middlewares/auth.ts` — deliberately not an `AuthPayload`/session token, carries a `purpose` field and no expiry so it keeps working indefinitely from an old email) encoding the (deceased person, recipient) pair. A new public route, `POST /api/memory-prompts/unsubscribe` (`routes/memories.ts`), verifies the token and inserts into the existing `memory_prompt_optouts` table — same effect as the pre-existing authenticated opt-out endpoint, just reachable without a session. New frontend page `memory-prompts-unsubscribe.tsx` (`/memory-prompts/unsubscribe?token=...`) auto-fires on load, same pattern as `verify-email.tsx`, and shows a plain confirmation. Full workspace typecheck clean; also verified the real esbuild bundle (`node build.mjs`) succeeds, not just `tsc`. **Not yet live-verified against a real inbox** — next step before trusting this fully is a throwaway deceased-test-profile + opt-in + click-the-real-email-link pass, same pattern used for the rest of the memories feature.
...
- [ ] Reply-to-email ingestion for the memories feature — was the spec'd *primary* contribution channel (60+ demographic, app-literacy risk) but the 2026-07-20 build deviated to deep-link-only; confirmed still open 2026-08-14, see `legacy_memories_feature.md`. Only real gap remaining from the original spec.
