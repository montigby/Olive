...
**2026-08-04, cron plan-limit check + a real rate-limit bug found and fixed (commit `3adcf21`)** — user manually ran `/api/cron/memory-prompts` from the Vercel dashboard (Cron Jobs settings page) to close the last open item from the memories-feature verification above. Confirmed: **Vercel Hobby plan, 2/2 cron job slots already used** (`birthday-emails`, `memory-prompts`) — no headroom for a third without upgrading to Pro or removing one, but both current jobs are fine. The manual run itself returned 200 (cron path confirmed working end-to-end, not just the immediate-on-opt-in path).

While reading that run's logs, found something unrelated but real: `express-rate-limit` was logging `ERR_ERL_KEY_GEN_IPV6` validation errors on every request. Traced it — `auth.ts`'s login rate limiter (10 attempts/10min) always keyed on raw `req.ip` (login is pre-auth, no personId to key on instead), string-interpolated without normalization. Since IPv6 users can trivially rotate through many addresses within their own assigned prefix, this fully defeated the brute-force cap for any IPv6-connected attacker — a live, exploitable gap in a protection `security.md` already documents as closed. `ai.ts`'s AI-chat limiter had the identical pattern but lower real risk (`requireAuth` runs first, so the IP fallback is normally unreachable). Fixed both using `express-rate-limit`'s own `ipKeyGenerator` helper, which normalizes IPv6 to a subnet prefix instead of the raw address. Typechecked clean, deployed, not yet independently re-verified live (would need a real IPv6 rotation test, which isn't practical to simulate — the fix directly matches the library's own documented remediation for this exact validation error, so confidence is high without that).

**2026-08-04 backlog closeout (commit `74d55c4`)** — user gave a hard "close out everything code-only in the known backlog" push. Five items fixed in parallel via background worktree agents, each diff independently re-verified (not just trusted from agent summaries) before merge, full workspace typecheck clean, committed as one and pushed:
- **Register flow now syncs new admins into the relationship graph** (`auth.ts`) — this was HANDOFF.md's stale "Priority 1" item, confirmed via grep it really was still missing, not just stale docs. New admins now get mirrored into the `people` table on `/auth/register` the same way `members.ts`/`ai.ts` already do for everyone else added later.
- **Dead code removed**: `layoutPersonalView` (475 lines) deleted from `tree.tsx` — confirmed zero callers first.
- **Mobile "Add Family Member" keyboard-cover bug — actually root-caused, not just mitigated.** Radix's `onOpenAutoFocus` was focusing the First Name field the instant the dialog mounted, before its own 200ms open animation finished — the OS keyboard's scroll-into-view logic then used mid-animation geometry, covering the field on the first tap (self-correcting on the second tap because the animation had long since settled by then, which matches the originally-reported symptom exactly). Fixed by deferring focus 250ms in `members.tsx`. Not live-device-verified yet (no local DATABASE_URL, and this needs a real phone) — reasoning is solid but flag if it doesn't hold up in practice.
- **Two dependency CVEs patched** (`pnpm-workspace.yaml` overrides, same pattern as `32bc868`): `ip-address` (high+moderate, SSRF/trust-boundary bypass) — genuinely production-relevant since it's inside `express-rate-limit`'s dependency chain, the package backing login/AI-chat rate limiting; and `esbuild` (low, dev-server-only).
- **Found and fixed, independent of the above**: `pnpm-workspace.yaml` had a broken literal placeholder (`esbuild: set this to true or false`) sitting in committed config since `32bc868` (2026-07-21) — not valid YAML boolean, would fail `ERR_PNPM_IGNORED_BUILDS` on any completely fresh `pnpm install`. Never caught locally because cached `node_modules` masked it; two independent background agents both hit it fresh in their worktrees and flagged it. Unclear whether this ever actually broke a Vercel deploy (site's been live and deploying fine since), but it's a real landmine now fixed to `esbuild: true`.
- **Full `orval codegen` regen actually run** for the first time (previous 2026-07-22 attempt was reverted as out-of-scope, see `openapi_drift.md` memory). Surfaced one real bug the old hand-patches had been silently papering over: 3 endpoints (waitlist signup, invite-merge preview/confirm) had `openapi.yaml` schema names that didn't match what orval's real operationId-based naming convention produces (`WaitlistSignupBody`/`MergePreviewBody`/`MergeConfirmBody` vs. what orval actually generates, `JoinWaitlistBody`/`PreviewInviteMergeBody`/`ConfirmInviteMergeBody`). Renamed the 3 schemas in the spec to match convention (not just patched call sites around the mismatch, so future regens won't hit the same landmine), updated the 5 call sites (`invites.ts`, `waitlist.ts`, `invite-claim.tsx`). Verified via grep that every previously-drifted-and-since-hand-patched field (venmo/snapchat/bereal, gender/deceased/viewerRelationshipLabel, birthdayCount/phoneCount, the full merge type set, all `hide*` fields, etc.) survived the regen intact. `openapi_drift.md`'s long-open item is now genuinely closed, not just safely deferred.

**Deliberately left out of this closeout** (flagged to user, needs either their input or a product decision, not something to guess at in the background):
- Cousin / great-grandchild / spouse's-parent relative-picker cases — no backend edge logic exists for these at all, real relationship-semantics ambiguity (see `suggestions_shortlist.md` item #26).
- Home page real life-events feed, and the deeper Recent-Updates diff-tracking fix — both flagged in the backlog as unapproved product-scope decisions, not unfinished code.
- Memories feature end-to-end verification, and the cross-family "shut out entirely" visibility tier check — need live browser testing against production with throwaway accounts, not background-agent-safe.
- Vercel cron job plan-limit check, `privacy@myolive.app` DNS forwarding, real landing-page photography, legal business entity status — all need the user directly, not something Claude can check or do.

Business model stayed explicitly parked per the user's instruction — nothing here touches billing/monetization.
...
**Still open:**
- [ ] Confirm the *daily cron sweep itself* still fires correctly for ongoing rotation (the immediate-send path is now verified; the cron path reuses the same shared function so should behave identically, but the cron's own trigger — needs `CRON_SECRET` — hasn't been separately re-verified since the refactor)
...
- **DB connection disables TLS certificate validation process-wide — NOT YET FIXED, needs a decision.** Found 2026-08-04 while reading Vercel cron logs: `lib/db/src/index.ts:15` passes `ssl: { rejectUnauthorized: false }` to the Postgres pool for connecting to Supabase. No code sets `NODE_TLS_REJECT_UNAUTHORIZED` directly, but `pg`'s SSL handling does so internally as a side effect of that option — meaning cert validation is disabled for the *entire server process*, not just the DB connection, for the lifetime of that instance. Every other outgoing HTTPS call from the same instance (OpenAI, Resend) loses certificate validation too. Pre-existing since the original DB setup, not introduced recently. Proper fix (using Supabase's real CA cert instead of blanket-disabling validation) carries real risk of breaking DB connectivity if rushed — flagged, not touched, pending the user's call on priority.
