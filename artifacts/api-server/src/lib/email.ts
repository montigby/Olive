...
// Exported so routes/webhooks.ts can reuse the same client construction
// (and lazy import, avoiding a top-level `resend` import in a route that
// needs to stay light) for webhook signature verification and fetching a
// received email's full content.
...
// Domain dedicated to inbound reply-to-email ingestion (routes/webhooks.ts)
// -- deliberately not myolive.app itself, so its MX record can't collide
// with the root domain's existing privacy@ forwarding (see
// email_forwarding_privacy in the auto-memory system).
const REPLY_DOMAIN = "reply.myolive.app";
...
export async function sendMemoryPrompt({
  ...
  personId: string;
  promptText: string;
  // The memory_prompt_log row id this send corresponds to (pre-generated by
  // the caller). Embedded in the Reply-To address so a plain "hit reply" in
  // any email client round-trips it back to routes/webhooks.ts without the
  // recipient ever visiting Olive -- the spec's original low-friction ask.
  // Deliberately a short, unprefixed identifier (not a signed JWT, unlike
  // the unsubscribe link): RFC 5321 caps an email local-part at 64 octets,
  // well under what a JWT needs but plenty for "mp-<uuid>".
  replyToken: string;
}) {
  const client = await getClient();
  ...
  const replyTo = `mp-${replyToken}@${REPLY_DOMAIN}`;
  const { error } = await client.emails.send({
    from: FROM,
    to,
    replyTo,
    ...
    text: `Hi ${recipientName},\n\n${promptText}\n\nJust hit reply and tell us about it -- no need to log in or visit the site.\n\nOr share it on the site: ${link}\n\nDon't want prompts about this person anymore? One click, no login needed: ${unsubscribeLink}\n\n— Olive\nhttps://myolive.app`,
  });

  if (error) throw new Error(`Resend error: ${JSON.stringify(error)}`);
}

export async function sendMemoryReplyConfirmation({
  to,
  recipientName,
  personName,
  personId,
  ...
  const { error } = await client.emails.send({
    from: FROM,
    to,
    subject: `Got it — thanks for sharing a memory of ${personName}`,
    html: buildMemoryReplyConfirmationHtml(recipientName, personName, link),
    text: `Hi ${recipientName},\n\nThanks for sharing that memory of ${personName} -- it's been added to their page for the rest of the family to see.\n\nView it here: ${link}\n\n— Olive\nhttps://myolive.app`,
  });
...
function buildMemoryReplyConfirmationHtml(recipientName: string, personName: string, link: string): string {
  const body = `
    <p style="margin:0 0 12px 0;">Hi ${recipientName},</p>
    <p style="margin:0 0 4px 0;">Thanks for sharing that memory of ${personName} -- it's been added to their page for the rest of the family to see.</p>
    ${button("View it on Olive", link)}
  `;
  return renderEmailShell({ preheader: `Thanks for sharing a memory of ${personName}`, bodyHtml: body });
}
