...
import { MailPlus, UserPlus, CheckCircle2, ChevronRight, Copy, Link as LinkIcon, Users } from "lucide-react";
import {
  Form,
  FormControl,
  FormField,
...
export default function Members() {
  ...
  const onSubmit = (data: AddMemberForm) => {
    addMemberMutation.mutate({ unitId, data }, {
      onSuccess: () => {
  ...
  const handleGenerateInvite = (personId: string) => {
    generateInviteMutation.mutate({ unitId, personId }, {
      onSuccess: (data) => {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
            ...
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  Add a member to your directory. You can invite them to claim this profile later.
                </DialogDescription>
              </DialogHeader>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="Son, Daughter, Grandpa..." {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
                  <Button type="submit" className="w-full mt-4" disabled={addMemberMutation.isPending}>
      ...
      <div className="grid gap-4">
        {isLoading ? (
          ...
          <Card key={member.id} className="border-none shadow-sm bg-card hover:bg-secondary/20 transition-colors group overflow-hidden">
            <CardContent className="p-0">
                ...
                <div className="flex items-center gap-2">
                  {!member.claimed && user?.isAdmin && (
                    inviteTokenMap[member.id] ? (
                      <Button variant="outline" size="sm" onClick={() => copyToClipboard(inviteTokenMap[member.id])} className="text-xs hidden sm:flex">
              ...
              {/* Mobile invite action below main row if unclaimed */}
              {!member.claimed && user?.isAdmin && (
                <div className="sm:hidden px-4 pb-4 pt-1">
                  {inviteTokenMap[member.id] ? (
                    <Button variant="outline" size="sm" onClick={() => copyToClipboard(inviteTokenMap[member.id])} className="w-full text-xs">
                      <Copy className="w-3 h-3 mr-2" /> Copy Link
                    </Button>
                  ) : (
                    <Button 
